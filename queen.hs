...
generateDiag x@(x1, x2) = concatMap (gen x) $ (,) <$> [1,-1] <*> [1,-1]
    where gen c@(c1, c2) s@(s1, s2) = let n = (c1 + s1, c2 + s2)
                                        in case n of
                                            (a,_) | a < 0 || a > boardSize -> []
                                            (_,a) | a < 0 || a > boardSize -> []
                                            _ -> n:(gen n s) 
...
getNextQueen queens r = [next:queens | next <- 
                    filter (\c -> not $ any (canAttack c) queens) ((,) <$> [r] <*> [0..boardSize]) ]

-- If you truly want all solutions (every assignement for every possible queens)
getNextQueen2 :: [(Int,Int)] -> Int -> [[(Int,Int)]]
getNextQueen2 queens r = [next:queens | next <- 
                    filter (\c -> not $ any (canAttack c) queens) allCells ]
